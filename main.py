...
# Описание: Это программа для распознавания лиц на изображениях и сравнения их
# с известными лицами. Она использует библиотеку face_recognition для
# обнаружения и кодирования лиц на фотографиях.
...
def save_similar_faces(known_faces, output_folder, recognized_faces):
    ...
    for i, known_face in enumerate(known_faces):
        ...
        for j, recognized_face in enumerate(recognized_faces):
            # Сравнение лиц
            if compare_faces(
                known_face["encoding"], np.array(recognized_face["encoding"])
            ):
                # Сохранение похожего лица в папку
                image_path = recognized_face[
                    "name"
                ]  # Путь к изображению с распознанным лицом
                _, image_filename = os.path.split(image_path)
                save_image_path = os.path.join(
                    known_face_folder, f"similar_{image_filename}"
                )
...
def recognize_faces_in_folder(folder_path, output_folder, known_faces):
    ...
    for image_file in image_files:
        ...
        for (top, right, bottom, left), face_data in zip(
...
known_faces = []
known_face_files = [
    f for f in os.listdir(known_faces_folder) if f.lower().endswith(".json")
]
